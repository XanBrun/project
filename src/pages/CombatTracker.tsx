...
import { 
  ...
  UserPlus, Bot, Settings, Download, Upload, RefreshCw
} from 'lucide-react';
import { 
...
function CombatTracker() {
  ...
  const updateParticipantHP = async (participantId: string, newHP: Partial<HitPoints>) => {
    if (!currentEncounter) return;
    ...
    try {
      const updatedParticipants = currentEncounter.participants.map(p => {
        if (p.id === participantId) {
          const conditions = p.conditions.includes(condition)
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {currentEncounter ? (
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-12 shadow-xl border border-amber-200 text-center">
          ...
          <p className="text-amber-700 mb-6">Crea un nuevo encuentro para comenzar el combate</p>
          <button
            onClick={() => setShowCreateModal(true)}
      ...
      {/* Initiative Modal */}
      {showInitiativeModal && currentEncounter && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
